...
**Author:** drafted by Claude Fable 5.1 from the user's brief; reviewed before execution.
...
- Where this plan says "decide," make the call, state it in the commit message,
  and move on. Where it says "ask the user," stop and ask.

---

## 1. Phase 0 — Discovery (ask the user; half a session)
...
- **Money-weighted return (XIRR):** solve `Σ CFᵢ/(1+r)^(tᵢ/365) = 0` with BMV as
  ...
  This is the investor's actual experience.
...
Build in this order; stop after the one that covers the real data (Phase 0 tells you which).
...
|---|---|---|
| 0 | Decisions block in this file; sample exports under `backend/data/investments/` | User has answered §1 |
...
| 3 | Import: manual snapshots + whichever of QIF/QFX/CSV Phase 0 selected; benchmark CSV upload | The real export imports with zero unexplained errors; re-import is idempotent |
...
| Kind | Count | Notes |
|---|---|---|
| 529 | 3 | Two beneficiaries, one duplicate |
| HSA | 4 | Two account numbers, each appearing twice (probable duplicates) |
| IRA | 2 | Traditional + Roth |
| 401(k) | 2 | Same employer, probable duplicate (name variants) |
| Brokerage | 2 | One TOD-registered, one may be a different registration |

**F3 — Only one account is taxable.** The TOD-registered brokerage account is the
...
**F4 — Four probable duplicate account pairs.** Two HSA account numbers each appear
twice, the 401(k) appears twice under name variants, and the brokerage appears
twice (one TOD-registered, one plain — may be genuinely different). Importing
positions before merging these will double-count the portfolio. **Resolve before
Phase 3.**

**F5 — No investment activity has leaked into the bank ledger.** One account that
looked like a brokerage by name is actually a credit card (annual membership fee,
late fees, interest charges). So the separate-domain design in §2.1 needs no data
cleanup or back-migration. Confirmed clean.
...
**F7 — Encoding damage in imported names.** Two credit-card account names show
mojibake (registered trademark symbols mangled) from the QIF import decoding
cp1252 bytes as UTF-8. Cosmetic, unrelated to investments, worth a separate
one-line fix.

### 9.2 Decisions taken by the agent
...
### 9.3 Decisions taken with the user (2026-09-09)
...
- **D8 (Q1/Q2) — Pursuing the Quicken QIF path.** The user exports the taxable
  brokerage account (the most demanding test) with *Investment transactions*,
  *Security lists* and *Prices* checked, into `backend/data/investments/`.
...
One brokerage account exported. Inspector summary (no amounts recorded here):
...
- Month-end valuations must then come from one of: manual statement snapshots
  ...
  upload for 16 symbols is not. The user decides whether to enable it.
